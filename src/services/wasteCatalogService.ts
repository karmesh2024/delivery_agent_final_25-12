...
class WasteCatalogService {
  // إضافة مخلفات جديدة
  async addWaste(waste: Omit<WasteCatalogItem, 'id' | 'created_at'>): Promise<WasteCatalogItem | null> {
    try {
      ...
      console.error('خطأ في إضافة المخلفات:', error);
      toast.error(`حدث خطأ أثناء إضافة المخلفات: ${error instanceof Error ? error.message : 'خطأ غير معروف'}`);
      return null;
    }
  }

  // تحديث مخلفات موجودة
  async updateWaste(id: number, waste: Partial<WasteCatalogItem>): Promise<WasteCatalogItem | null> {
    try {
      const { data, error } = await supabase!
        .from('catalog_waste_materials')
        .update(waste)
      ...
      console.error('خطأ في تحديث المخلفات:', error);
      toast.error(`حدث خطأ أثناء تحديث المخلفات: ${error instanceof Error ? error.message : 'خطأ غير معروف'}`);
      return null;
    }
  }
  ...
  async deleteWasteMaterial(id: number): Promise<boolean> {
    try {
      ...
      console.error('خطأ في حذف المخلفات:', error);
      toast.error(`حدث خطأ أثناء حذف المخلفات: ${error instanceof Error ? error.message : 'خطأ غير معروف'}`);
      return false;
    }
  }
  ...
  async getWasteSubCategories(mainCategoryId?: number): Promise<WasteSubCategory[]> {
    try {
      let query = supabase!
  ...
  async addWasteMainCategory(code: string, name: string): Promise<WasteMainCategory | null> {
    try {
      const { data, error } = await supabase!
        .from('waste_main_categories')
        .insert([{ code, name }])
      ...
      console.error('خطأ في إضافة فئة المخلفات الأساسية:', error);
      toast.error(`حدث خطأ أثناء إضافة الفئة: ${error instanceof Error ? error.message : 'خطأ غير معروف'}`);
      return null;
    }
  }

  // إضافة فئة فرعية جديدة للمخلفات
  async addWasteSubCategory(code: string, name: string, mainId: number): Promise<WasteSubCategory | null> {
    try {
      const { data, error } = await supabase!
        .from('waste_sub_categories')
      ...
      console.error('خطأ في إضافة فئة المخلفات الفرعية:', error);
      toast.error(`حدث خطأ أثناء إضافة الفئة الفرعية: ${error instanceof Error ? error.message : 'خطأ غير معروف'}`);
      return null;
    }
  }

  // التحقق من وجود رقم المخلفات
  async checkWasteNumberExists(wasteNo: string, excludeId?: number): Promise<boolean> {
    try {
      let query = supabase!
        .from('catalog_waste_materials')
        .select('id')
  ...
  async searchRelatedProducts(query: string): Promise<Array<{id: number, name: string, sku: string, brand: string}>> {
    try {
      const { data, error } = await supabase!
  ...
  calculateEnvironmentalSavings(wasteType: string, weight: number): {
    ...
    const coefficients: { [key: string]: { carbon: number, energy: number, water: number, trees: number } } = {
  ...
  async updateWasteStatus(id: number, status: string): Promise<boolean> {
    try {
      ...
      console.error('خطأ في تحديث حالة المخلفات:', error);
      toast.error(`حدث خطأ أثناء تحديث الحالة: ${error instanceof Error ? error.message : 'خطأ غير معروف'}`);
      return false;
    }
  }
  ...
  async getWasteStatistics(): Promise<{
    ...
    try {
      const { data, error } = await supabase!
        .from('catalog_waste_materials')
        .select('weight, volume, count, recyclable, expected_total, main_category_id');
      ...
      data?.forEach(item => {
        ...
        categoriesBreakdown[categoryId] = (categoriesBreakdown[categoryId] || 0) + quantity;
      });
  ...
  async getUnits(): Promise<{ id: number; name: string; symbol: string }[]> {
    try {
      ...
      if (error) {
        console.warn('خطأ في جلب الوحدات من قاعدة البيانات، استخدام البيانات الافتراضية:', error.message);
        // إرجاع بيانات افتراضية في حالة عدم وجود الجدول
        return this.getDefaultUnits();
      }

      if (data && data.length > 0) {
        console.log('تم جلب الوحدات بنجاح من قاعدة البيانات:', data.length, 'وحدة');
        return data;
      } else {
        console.warn('لا توجد وحدات في قاعدة البيانات، استخدام البيانات الافتراضية');
        return this.getDefaultUnits();
      }
    } catch (error) {
      console.warn('خطأ في الاتصال بقاعدة البيانات، استخدام البيانات الافتراضية:', error instanceof Error ? error.message : 'خطأ غير معروف');
  ...
  private getDefaultSourceOptions(): { id: string; name: string; description?: string }[] {
    return [
      {
        id: 'damaged_product',
        name: 'منتج تالف',
  ...
  async getSourceOptions(filters?: { sectorId?: number; clientTypeId?: number }): Promise<{ id: string; name: string; description?: string }[]> {
    try {
      // إذا تم تمرير فلتر القطاع ونوع العميل، استخدم النظام الجديد
      if (filters?.sectorId && filters?.clientTypeId) {
        console.log('استخدام النظام الجديد لجلب خيارات المصدر للقطاع', filters.sectorId, 'ونوع العميل', filters.clientTypeId);
        const availableSources = await this.getAvailableSources(filters.sectorId, filters.clientTypeId);
      ...
      if (error) {
        console.warn('خطأ في جلب خيارات المصدر من قاعدة البيانات، استخدام البيانات الافتراضية:', error.message);
        return this.getDefaultSourceOptions();
      }

      if (data && data.length > 0) {
        console.log('تم جلب خيارات المصدر بنجاح من قاعدة البيانات:', data.length, 'خيار');
        return data;
      } else {
        console.warn('لا توجد خيارات مصدر في قاعدة البيانات، استخدام البيانات الافتراضية');
        return this.getDefaultSourceOptions();
      }
    } catch (error) {
      console.warn('خطأ في الاتصال بقاعدة البيانات، استخدام البيانات الافتراضية:', error instanceof Error ? error.message : 'خطأ غير معروف');
  ...
  async getWasteSectors(): Promise<WasteSector[]> {
    try {
      ...
      const uniqueSectors = data?.reduce((acc: WasteSector[], item: { sector_code: string }) => {
  ...
  async getAvailableSources(sectorId: number, clientTypeId: number): Promise<WasteMainCategory[]> {
    try {
      const { data, error } = await supabase!
      ...
      return data?.map((item: AvailableSourceData) => {
        ...
        const source = Array.isArray(item.waste_sources) ? item.waste_sources[0] : item.waste_sources;
        return {
  ...
  async getAvailableReasons(sectorId: number, clientTypeId: number, sourceId: string): Promise<SourceReason[]> {
    try {
      const { data, error } = await supabase!
        .from('sector_client_source_reason')
        .select(`
      ...
      return data?.map((item: AvailableReasonData) => {
        ...
        const reason = Array.isArray(item.source_reasons) ? item.source_reasons[0] : item.source_reasons;
        return {
          id: reason.id,
          name: reason.name,
  ...
  private async getSourceInfoById(sourceId: string): Promise<{ id: string; name: string; description?: string } | null> {
    try {
  ...
  // إضافة قطاع جديد
  async addWasteSector(sector: { name: string; description?: string }): Promise<WasteSector | null> {
    try {
      const { data, error } = await supabase!
        .from('waste_sectors')
        .insert([sector])
      ...
      console.error('خطأ في إضافة القطاع:', error);
      toast.error(`حدث خطأ أثناء إضافة القطاع: ${error instanceof Error ? error.message : 'خطأ غير معروف'}`);
      return null;
    }
  }

  // إضافة نوع عميل جديد
  async addClientType(clientType: { sector_id: number; name: string; description?: string }): Promise<ClientType | null> {
    try {
      const { data, error } = await supabase!
        .from('client_types')
        .insert([clientType])
      ...
      console.error('خطأ في إضافة نوع العميل:', error);
      toast.error(`حدث خطأ أثناء إضافة نوع العميل: ${error instanceof Error ? error.message : 'خطأ غير معروف'}`);
      return null;
    }
  }

  // إضافة مصدر مخلفات جديد
  async addWasteSource(source: { id: string; name: string; description?: string }): Promise<{ id: string; name: string; description?: string } | null> {
    try {
      ...
      if (error) {
        ...
        toast.error(`حدث خطأ أثناء إضافة مصدر المخلفات: ${error.message || 'خطأ غير معروف'}`);
        return null;
      }
  ...
  // إضافة سبب مصدر جديد
  async addSourceReason(reason: { name: string; description?: string }): Promise<SourceReason | null> {
    try {
      ...
      if (error) {
        ...
        toast.error(`حدث خطأ أثناء إضافة سبب المصدر: ${error.message || 'خطأ غير معروف'}`);
        return null;
      }
  ...
  async addSectorClientSourceReasonLink(link: { 
    ...
    try {
      ...
      console.error('خطأ في إضافة الربط:', error);
      toast.error(`حدث خطأ أثناء إضافة الربط: ${error instanceof Error ? error.message : 'خطأ غير معروف'}`);
      return false;
    }
  }
}
