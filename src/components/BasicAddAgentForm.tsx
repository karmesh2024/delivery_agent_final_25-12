...
// تعريف واجهة ممتدة للتعامل مع الحقول الإضافية التي قد تكون موجودة في كائن الاستجابة
interface ExtendedDeliveryBoy extends DeliveryBoy {
  password?: string;
...
interface BasicAddAgentFormProps {
  onSuccess: () => void;
  onCancel: () => void;
}

export function BasicAddAgentForm({ onSuccess, onCancel }: BasicAddAgentFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newAgent = await createDeliveryBoy({
        ...
        status: formData.status === "active" ? "active" : "inactive", // تحويل إلى النوع المناسب
      });
      
      if (newAgent) {
        // استخدام الواجهة الممتدة للوصول إلى الخصائص الإضافية
        const extendedAgent = newAgent as ExtendedDeliveryBoy;
        setSuccessMessage(`تم إضافة المندوب ${formData.full_name} بنجاح`);
        
        // حفظ كلمة المرور من أي من الحقول المحتملة
        if (extendedAgent.password) {
          setCreatedPassword(extendedAgent.password);
        } else if (extendedAgent.generated_password) {
          setCreatedPassword(extendedAgent.generated_password);
        } else if (extendedAgent.user_password) {
          setCreatedPassword(extendedAgent.user_password);
        } else if (extendedAgent.tempPassword) {
          setCreatedPassword(extendedAgent.tempPassword);
        }
        
        // حفظ كود التسليم
        if (extendedAgent.delivery_code) {
          setCreatedDeliveryCode(extendedAgent.delivery_code);
        } else if (extendedAgent.deliveryCode) {
          setCreatedDeliveryCode(extendedAgent.deliveryCode);
        }
...
export default BasicAddAgentForm; 