...
import { logger, apiLogger, dbLogger } from '@/lib/logger-safe';
import { cleanForLog } from '@/lib/logger-safe';
...
export const warehouseService = {
  ...
  async isAdminWarehouse(warehouseId: number): Promise<boolean> {
    try {
      ...
      return warehouse ? (warehouse.is_admin_warehouse === true || warehouse.warehouse_level === 'admin') : false;
    } catch {
      return false;
    }
  },
  ...
  getDefaultPermissions(level: WarehouseLevel): Array<{ permission_type: string; permission_value: boolean }>{
    const all: string[] = [
      'create_warehouse',
      'edit_warehouse',
      'delete_warehouse',
    ...
    const allow = (types: string[]) =>
      all.map(t => ({ permission_type: t, permission_value: types.includes(t) }));

    if (level === 'admin') return allow(all);
    if (level === 'country') return allow(['create_warehouse', 'edit_warehouse', 'view_reports']);
  ...
  async getWarehouseLevelCode(warehouseId: number): Promise<WarehouseLevel | null> {
    try {
      ...
      if (warehouseError) {
        console.error('getWarehouseLevelCode - خطأ من Supabase (warehouses):', warehouseError);
      }
      
      // إذا كان هناك warehouse_level مباشرة (VARCHAR)
      if (warehouseData?.warehouse_level) {
        console.log('getWarehouseLevelCode - warehouse_level من warehouses:', warehouseData.warehouse_level);
        return warehouseData.warehouse_level as WarehouseLevel;
      }
      
      // إذا كان هناك level_id (UUID)
      if (warehouseData?.level_id) {
        console.log('getWarehouseLevelCode - level_id من warehouses:', warehouseData.level_id);
        const levelCode = await this.getLevelCodeById(String(warehouseData.level_id));
        console.log('getWarehouseLevelCode - levelCode:', levelCode);
        return levelCode;
      }
      
      // محاولة ثانية: من جدول warehouse_hierarchy
      console.log('getWarehouseLevelCode - محاولة جلب المستوى من warehouse_hierarchy');
      ...
      if (hierarchyError) {
        console.error('getWarehouseLevelCode - خطأ من Supabase (warehouse_hierarchy):', hierarchyError);
      }
      
      if (hierarchyData?.level?.code) {
        console.log('getWarehouseLevelCode - levelCode من warehouse_hierarchy:', hierarchyData.level.code);
        return hierarchyData.level.code as WarehouseLevel;
      }
      
      if (hierarchyData?.level_id) {
        console.log('getWarehouseLevelCode - level_id من warehouse_hierarchy:', hierarchyData.level_id);
        const levelCode = await this.getLevelCodeById(String(hierarchyData.level_id));
        console.log('getWarehouseLevelCode - levelCode:', levelCode);
        return levelCode;
      }
      
      console.warn('getWarehouseLevelCode - لا يوجد level_id للمخزن في أي جدول:', warehouseId);
      return null;
    } catch (e) {
  ...
  async add(warehouse: Omit<Warehouse, 'id' | 'created_at' | 'updated_at'>) {
    try {
      ...
      if (warehouse.is_admin_warehouse === true || warehouse.warehouse_level === 'admin') {
        toast.error('لا يمكن إنشاء الإدارة العليا من هنا. يرجى استخدام صفحة إعدادات الإدارة العليا');
        return null;
      }

      // التحقق من القواعد الهرمية إذا كان هناك parent
      if (warehouse.parent_warehouse_id) {
        const parentWarehouse = await this.getById(warehouse.parent_warehouse_id);
        if (!parentWarehouse) {
        ...
        if (warehouse.warehouse_level && parentWarehouse.warehouse_level) {
          const allowedLevels = this.allowedChildLevels(parentWarehouse.warehouse_level as WarehouseLevel);
          if (!allowedLevels.includes(warehouse.warehouse_level as WarehouseLevel)) {
            toast.error(`المستوى ${warehouse.warehouse_level} غير مسموح تحت المستوى ${parentWarehouse.warehouse_level}`);
            return null;
          }
        }
      }
  ...
  async update(id: number, warehouse: Partial<Warehouse>) {
    try {
      ...
      if (isAdmin) {
        toast.error('لا يمكن تعديل الإدارة العليا من هنا. يرجى استخدام صفحة إعدادات الإدارة العليا');
        return null;
      }
      ...
      if (warehouse.parent_warehouse_id !== undefined) {
        ...
        if (currentWarehouse?.is_admin_warehouse || currentWarehouse?.warehouse_level === 'admin') {
  ...
  async getSectors(): Promise<Sector[]> {
    try {
      ...
      // تحويل البيانات إلى تنسيق Sector مع إضافة ID فريد
      const uniqueSectors = data?.reduce((acc: Sector[], item: any, index: number) => {
        const existingSector = acc.find(s => s.code === item.sector_code);
        if (!existingSector) {
          const sector: Sector = {
            id: `sector_${index + 1}`, // ID فريد للقطاع
          ...
          acc.push(sector);
          console.log('تم إضافة القطاع:', sector);
        }
        return acc;
      }, []) || [];
      
      console.log('القطاعات النهائية:', uniqueSectors);
      console.log(`إجمالي القطاعات: ${uniqueSectors.length}`);
  ...
  // ربط مخزن بقطاع
  async addWarehouseSector(warehouseId: number, sectorCode: string, isPrimary: boolean = false, capacityPercentage: number = 100): Promise<boolean> {
    try {
      const { error } = await supabase!
        .from('warehouse_sectors')
        .insert([{
  ...
  async removeWarehouseSector(warehouseId: number, sectorCode: string): Promise<boolean> {
    try {
      const { error } = await supabase!
        .from('warehouse_sectors')
        .delete()
  ...
  async createHierarchicalWarehouse(warehouse: Omit<Warehouse, 'id' | 'created_at' | 'updated_at'>, sectors?: string[]): Promise<Warehouse | null> {
    try {
      // إنشاء المخزن
      const payload = {
        name: warehouse.name,
      ...
      if (sectors && sectors.length > 0) {
        for (const sectorCode of sectors) {
          await this.addWarehouseSector(newWarehouse.id, sectorCode, sectorCode === sectors[0]); // الأول أساسي
        }
      }
  ...
  async getAdminWarehouse(includeInactive: boolean = false): Promise<Warehouse | null> {
    try {
      let query = supabase!
        .from('warehouses')
        .select('*')
    ...
    try {
      ...
      if (existingAdmin) {
        ...
        if (!existingAdmin.is_active) {
          ...
          toast.error('الإدارة العليا موجودة بالفعل. يمكن أن يكون هناك إدارة عليا واحدة فقط');
          return null;
        }
      }
  ...
  async updateAdminSettings(warehouseId: number, settings: Record<string, unknown>): Promise<boolean> {
    try {
      const { error } = await supabase!
        .from('warehouses')
  ...
  async updateFunctionalStructure(warehouseId: number, structure: Record<string, unknown>): Promise<boolean> {
    try {
      const { error } = await supabase!
        .from('warehouses')
  ...
  async updateSystemConfiguration(warehouseId: number, config: Record<string, unknown>): Promise<boolean> {
    try {
      const { error } = await supabase!
        .from('warehouses')
    ...
    try {
      ...
      if (result.error) {
        ...
        return result.data.map((item: any) => ({
          ...
          sector_name: item.product_classifications?.warehouse_sectors?.name || 'غير محدد',
          sector_code: item.product_classifications?.warehouse_sectors?.code || 'UNKNOWN',
          sector_color: item.product_classifications?.warehouse_sectors?.color || '#6B7280'
            }));
          } else {
    ...
    try {
      ...
      if (result.error) {
        ...
        return result.data.map((item: any) => ({
          ...
          classification_name: item.main_categories?.product_classifications?.name || 'غير محدد',
          sector_name: item.main_categories?.product_classifications?.warehouse_sectors?.name || 'غير محدد',
          sector_code: item.main_categories?.product_classifications?.warehouse_sectors?.code || 'UNKNOWN',
          sector_color: item.main_categories?.product_classifications?.warehouse_sectors?.color || '#6B7280'
        }));
            } else {
  ...
  async getSectors(): Promise<any[]> {
    try {
      ...
      if (result.data && result.data.length > 0) {
        console.log(`تم جلب ${result.data.length} قطاع من جدول warehouse_sectors`);
        return result.data;
      } else {
        console.log('لا توجد قطاعات في جدول warehouse_sectors، سيتم إنشاء بيانات افتراضية');
  ...
  async getProductCategories(): Promise<any[]> {
    try {
      ...
      if (result.error) {
        ...
        console.log('لا توجد فئات منتجات في قاعدة البيانات، سيتم إنشاء بيانات افتراضية');
    ...
    try {
      ...
      if (result.error) {
        ...
        throw new Error(`خطأ في قاعدة البيانات: ${result.error.message}`);
          ...
          if (result.data && result.data.length > 0) {
        console.log(`تم جلب ${result.data.length} فئة مخلفات من قاعدة البيانات`);
        return result.data.map((item: any) => ({
          ...item,
          sector_name: item.warehouse_sectors?.name || 'غير محدد',
          sector_code: item.warehouse_sectors?.code || 'UNKNOWN',
        ...
        console.log('لا توجد فئات مخلفات في قاعدة البيانات، سيتم إنشاء بيانات افتراضية');
      ...
      const warehousesWithPermissions = await Promise.all(
        data.map(async (warehouse: any) => {
          try {
            ...
            if (!warehouse.permissions || warehouse.permissions.length === 0) {
              ...
              if (warehouse.warehouse_level) {
                ...
                try {
                  ...
                  console.warn(`تعذر جلب مستوى المخزن ${warehouse.id}:`, levelErr);
                }
              }
              
              if (levelCode && ['admin', 'country', 'city', 'district'].includes(levelCode)) {
                const defaultPermissions = this.getDefaultPermissions(levelCode);
                ...
                try {
                  const { error: permInsertError, data: insertedPerms } = await supabase!
                    .from('warehouse_permissions')
                  ...
                  if (!permInsertError && insertedPerms) {
                    ...
                    console.warn(`خطأ في إضافة الصلاحيات للمخزن ${warehouse.id}:`, permInsertError);
                  ...
                  console.warn(`خطأ في إضافة الصلاحيات الافتراضية للمخزن ${warehouse.id}:`, permErr);
                  // على الأقل نضيف الصلاحيات في الذاكرة للعرض
                  warehouse.permissions = defaultPermissions.map(perm => ({
                    id: `temp-${warehouse.id}-${perm.permission_type}`,
                    warehouse_id: warehouse.id,
            ...
            console.error(`خطأ في معالجة المخزن ${warehouse.id}:`, warehouseErr);
  ...
  async createWarehouse(warehouseData: {
    ...
    try {
      ...
      // تحقق السماح الهرمي
      const requestedLevelCode = await this.getLevelCodeById(warehouseData.level_id);
      ...
      if (requestedLevelCode === 'country') {
        ...
        if (!warehouseData.parent_warehouse_id) {
          ...
          finalParentWarehouseId = typeof admin.id === 'number' ? admin.id : Number(admin.id);
        } else {
          finalParentWarehouseId = Number(warehouseData.parent_warehouse_id);
          // تحقق أن الأب فعلاً إدارة عليا
          const parentLevel = await this.getWarehouseLevelCode(finalParentWarehouseId);
      ...
      if (requestedLevelCode === 'city' || requestedLevelCode === 'district') {
        if (!warehouseData.parent_warehouse_id) {
          toast.error(requestedLevelCode === 'city'
      ...
      if (finalParentWarehouseId) {
        const parentLevelCode = await this.getWarehouseLevelCode(finalParentWarehouseId);
        if (!parentLevelCode) {
      ...
      if (newId) {
        ...
        const { error: permError } = await supabase!.from('warehouse_permissions').insert(rows);
        if (permError) {
          console.warn('createWarehouse - permissions insert error:', permError.message);
        }
      }
  ...
  async updateWarehouse(warehouseId: string, warehouseData: {
    ...
    try {
      ...
      if (!permissionCheck.allowed) {
        toast.error(permissionCheck.reason || 'ليس لديك صلاحية لتعديل هذا المخزن');
        return false;
      }
  ...
  async createPermissionDelegation(delegationData: {
    ...
    try {
      ...
      console.log('createPermissionDelegation - جلب مستوى المخزن المفوض:', delegationData.delegator_warehouse_id);
      const delegatorLevel = await this.getWarehouseLevelCode(Number(delegationData.delegator_warehouse_id));
      console.log('createPermissionDelegation - مستوى المخزن المفوض:', delegatorLevel);
      
      // إذا لم نتمكن من تحديد المستوى، نسمح بالتفويض مع تحذير
      if (!delegatorLevel) {
        console.warn('createPermissionDelegation - تحذير: تعذر تحديد مستوى المخزن المفوض، سيتم المتابعة مع التحقق الأساسي');
        // نتابع العملية بدون فحص المستوى
      } else if (delegatorLevel === 'city' || delegatorLevel === 'district') {
        console.error('createPermissionDelegation - فشل: هذا المستوى لا يمكنه التفويض');
      ...
      if (userError) {
        console.error('createPermissionDelegation - خطأ في جلب المستخدم:', userError);
      }
      ...
      console.log('createPermissionDelegation - البيانات المرسلة إلى قاعدة البيانات:', insertData);
  ...
  async updatePermissionDelegation(
    ...
    try {
      console.log('updatePermissionDelegation - بدء العملية:', delegationId, delegationData);
      ...
      if (delegationData.delegator_warehouse_id !== undefined) {
        updateData.delegator_warehouse_id = delegationData.delegator_warehouse_id;
      }
      if (delegationData.delegatee_warehouse_id !== undefined) {
        updateData.delegatee_warehouse_id = delegationData.delegatee_warehouse_id;
      }
      ...
      if (delegationData.expires_at !== undefined) {
        updateData.expires_at = delegationData.expires_at ? new Date(delegationData.expires_at).toISOString() : null;
      }
  ...
  async getHierarchyReports(): Promise<{
    ...
    try {
      const [warehousesResult, delegationsResult, permissionsResult] = await Promise.all([
        supabase!.from('warehouses').select('id, level_id').eq('is_active', true),
        supabase!.from('permission_delegations').select('id').eq('is_active', true),
        supabase!.from('warehouse_permissions').select('permission_type, permission_value')
      ]);
      ...
      permissions.forEach(permission => {
        const key = `${permission.permission_type}_${permission.permission_value}`;
