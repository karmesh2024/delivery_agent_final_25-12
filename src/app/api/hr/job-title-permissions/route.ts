...
export async function POST(request: Request) {
  try {
    ...
    const newJobTitlePermission = await hrUseCases.grantPermissionToJobTitle(body);
    return NextResponse.json(newJobTitlePermission, { status: 201 });
  } catch (error: any) {
    console.error("Error in POST /api/hr/job-title-permissions:", error);
