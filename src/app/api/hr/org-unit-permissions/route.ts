...
export async function POST(request: Request) {
  try {
    ...
    const newOrgUnitPermission = await hrUseCases.grantPermissionToUnit(body);
    return NextResponse.json(newOrgUnitPermission, { status: 201 });
  } catch (error: any) {
    console.error("Error in POST /api/hr/org-unit-permissions:", error);
